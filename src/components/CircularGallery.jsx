...
class GalleryApp {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    this.setInitialScroll(galleryItems.length);
  }

  setInitialScroll(itemCount) {
    if (!this.medias || !this.medias[0]) return;
    const initialIndex = Math.max(1, Math.floor(itemCount / 2));
    const initialScroll = this.medias[0].width * initialIndex;
  ...
  update() {
    if (this.autoScrollSpeed && !this.isDown) {
